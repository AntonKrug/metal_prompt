...
bool M_P_CFG_COLOR_ENABLEd = true;
...
void m_p_color_buf_default(char* buf) {
    if (M_P_CFG_COLOR_ENABLEd) strcat(buf, "\033[0;39m");
}
...
void m_p_color_buf_default_bold(char* buf) {
    if (M_P_CFG_COLOR_ENABLEd) strcat(buf, "\033[1;39m");
}


M_P_CFG_FORCE_OPTIMIZATION
void m_p_color_buf_command_namespace(char* buf) {
    if (M_P_CFG_COLOR_ENABLEd) strcat(buf, "\033[1;35m");
}


M_P_CFG_FORCE_OPTIMIZATION
void m_p_color_buf_command_name(char* buf) {
    if (M_P_CFG_COLOR_ENABLEd) strcat(buf, "\033[1;33m");
}


M_P_CFG_FORCE_OPTIMIZATION
void m_p_color_buf_gray(char* buf) {
    if (M_P_CFG_COLOR_ENABLEd) strcat(buf, "\033[1;30m");
}
...
void m_p_color_out_default(void) {
    if (M_P_CFG_COLOR_ENABLEd) m_p_transport_out("\033[0;39m");
}


M_P_CFG_FORCE_OPTIMIZATION
void m_p_color_out_prompt(void) {
    if (M_P_CFG_COLOR_ENABLEd) m_p_transport_out("\033[1;36m");
}


M_P_CFG_FORCE_OPTIMIZATION
void m_p_color_out_error(void) {
    if (M_P_CFG_COLOR_ENABLEd) m_p_transport_out("\033[1;31m");
}
